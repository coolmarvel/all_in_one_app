...
contract OFTStakingMock is IOFTReceiver {
    ...
    function setRemoteStakingContract(
        uint16 _chainId,
        bytes calldata _stakingContract
    ) external {
        remoteStakingContracts[_chainId] = _stakingContract;
    }

    function deposit(uint _amount) external payable {
        IERC20(oft.token()).safeTransferFrom(
            msg.sender,
            address(this),
            _amount
        );
    ...
    function depositToDstChain(
        ...
        require(
            keccak256(dstStakingContract) != keccak256(""),
            "invalid _dstChainId"
        );
        ...
        oft.sendAndCall{value: msg.value}(
            address(this),
            _dstChainId,
            dstStakingContract,
            _amount,
    ...
    function onOFTReceived(
        ...
        require(
            msg.sender == address(oft),
            "only oft can call onOFTReceived()"
        );
        require(
            keccak256(_from) == keccak256(remoteStakingContracts[_srcChainId]),
            "invalid from"
        );
