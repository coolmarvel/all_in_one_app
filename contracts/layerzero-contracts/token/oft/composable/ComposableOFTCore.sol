...
abstract contract ComposableOFTCore is OFTCore, IComposableOFTCore {
    ...
    mapping(uint16 => mapping(bytes => mapping(uint64 => bytes32)))
        public failedOFTReceivedMessages;
    ...
    function supportsInterface(
        ...
        return
            interfaceId == type(IComposableOFTCore).interfaceId ||
    ...
    function retryOFTReceived(
        ...
        bytes32 msgHash = failedOFTReceivedMessages[_srcChainId][_srcAddress][
            _nonce
        ];
        require(
            msgHash != bytes32(0),
            "ComposableOFTCore: no failed message to retry"
        );

        bytes32 hash = keccak256(abi.encode(_from, _to, _amount, _payload));
        require(
            hash == msgHash,
            "ComposableOFTCore: failed message hash mismatch"
        );

        delete failedOFTReceivedMessages[_srcChainId][_srcAddress][_nonce];
        IOFTReceiver(_to).onOFTReceived(
            _srcChainId,
            _srcAddress,
            _nonce,
            _from,
    ...
    function _sendAndCallAck(
        ...
        (
            ...
            uint64 gasForCall
        ) = abi.decode(_payload, (uint16, bytes, bytes, uint, bytes, uint64));
    ...
    function _safeCallOnOFTReceived(
        ...
        if (!success) {
            failedOFTReceivedMessages[_srcChainId][_srcAddress][
                _nonce
            ] = keccak256(abi.encode(_from, _to, _amount, _payload));
            emit CallOFTReceivedFailure(
                _srcChainId,
                _srcAddress,
                _nonce,
                _from,
