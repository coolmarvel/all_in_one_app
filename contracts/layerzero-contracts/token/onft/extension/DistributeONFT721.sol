...
contract DistributeONFT721 is ONFT721 {
    ...
    constructor(
        ...
        require(
            _indexArrayLength == _valueArray.length,
            "_indexArray and _valueArray must be same length"
        );
    ...
    function distributeTokens(
        ...
        bytes memory _adapterParams = _getMultiAdaptParams(
    ...
    function getDistributeTokens(
        uint _amount
    ) external view returns (TokenDistribute[] memory) {
        ...
        TokenDistribute[] memory tokenDistributeFixed = new TokenDistribute[](
            tokenDistributeSize
        );
        uint index;
        for (uint i; i < tokenIdsLength; ) {
    ...
    function _nonblockingLzReceive(
        ...
        if (functionType == FUNCTION_TYPE_SEND) {
            ...
            (
                ,
                bytes memory _toAddressBytes,
                uint[] memory _receivedTokenIds
            ) = abi.decode(_payload, (uint16, bytes, uint[]));
            ...
            (, TokenDistribute[] memory tokenDistribute) = abi.decode(
                _payload,
                (uint16, TokenDistribute[])
            );
            ...
            for (uint i; i < tokenDistributeLength; ) {
                ...
                tokenIds[tokenDistribute[i].index] =
                    temp |
    ...
    function clearCredits(bytes memory _payload) external virtual override {
        ...
        (, , uint[] memory _tokenIds) = abi.decode(
            _payload,
            (uint16, bytes, uint[])
        );
    ...
    function _flipBits(TokenDistribute[] memory _tokenDistribute) internal {
        uint tokenDistributeLength = _tokenDistribute.length;
        for (uint i; i < tokenDistributeLength; ) {
            tokenIds[_tokenDistribute[i].index] =
                tokenIds[_tokenDistribute[i].index] ^
                _tokenDistribute[i].value;
            unchecked {
                ++i;
            }
        }
    ...
    function _estimatePayloadFee(
        ...
        return
            lzEndpoint.estimateFees(
                ...
                _payload,
                _useZro,
                _getMultiAdaptParams(_amount)
            );
    }

    function _countTokenDistributeSize(
        uint _amount
    ...
    function _getMultiAdaptParams(
        ...
        uint16 version = 1;
        uint destinationGas = distributeBaseDstGas +
            ((_amount - 1) * distributeGasPerIdx);
    ...
    function _getNextMintTokenIdAndClearFlag() internal returns (uint tokenId) {
        ...
        for (uint i; i < tokenIdsLength; ) {
            ...
            tokenId =
                (MAX_TOKENS_PER_INDEX - position) +
                (i * NUM_TOKENS_PER_INDEX);
            break;
        }
        return tokenId;
    }

    function setDistributeBaseDstGas(
        uint _distributeBaseDstGas
    ) external onlyOwner {
        distributeBaseDstGas = _distributeBaseDstGas;
        emit SetDistributeBaseDstGas(distributeBaseDstGas);
    }

    function setDistributeGasPerIdx(
        uint _distributeGasPerIdx
    ) external onlyOwner {
