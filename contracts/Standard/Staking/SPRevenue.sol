 ...
 * @dev Standard Staking Pool ver.Revenue
 */
contract SPRevenue is
    SPMineRevenue,
    SPCoolTime,
    SPLockUp,
    SPLimit,
    ...
    ReentrancyGuard
{
    ...
    ) SPMineRevenue(game, rt, st, service, recipientRole) {
    ...
    function _claim(
        address user
    ) internal virtual override(SPBase, SPMineRevenue) {
        super._claim(user);
    }
    ...
    function _beforeStake(
        address user,
        uint amount
    ) internal view virtual override(SPBase, SPCoolTime, SPLimit) {
