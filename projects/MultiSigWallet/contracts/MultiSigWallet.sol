...
contract MultiSigWallet is IMultiSigWallet {
  ...
  constructor(address[] memory _owners, uint256 _quorum) {
    require(_owners.length > 2, "Treasury: Number of owners must be greater than 2 to guarantee it is a voting system.");
    require(_quorum > 1 && _quorum <= _owners.length, "Treasury: Number of confirmations does not satisfy quorum.");
  ...
  function executeTransaction(uint256 _transactionId) public payable onlyOwner isTransactionExist(_transactionId) notExecuted(_transactionId) {
    ...
    require(transaction.currentNumberOfConfirmations >= quorum, "Treasury: Current Number Of Confirmations must be greater than or equal to quorum.");
  ...
  function revokeConfirmation(uint256 _transactionId) public onlyOwner isTransactionExist(_transactionId) notExecuted(_transactionId) {
    ...
    require(isConfirmed[_transactionId][msg.sender], "Treasury: Transaction is not confirmed");
   ...
   */
  function addOwner(address _newOwner) external onlyWallet notNull(_newOwner) isNotOneOfOwner(_newOwner) validRequirement(owners.length + 1, quorum) {
   ...
   */
  function removeOwner(address _owner) external onlyWallet isOneOfOwner(_owner) {
   ...
   * @param _newOwner Owner address to add.
   */
  function replaceOwner(address _owner, address _newOwner) external onlyWallet isOneOfOwner(_owner) isNotOneOfOwner(_newOwner) {
   ...
   */
  function changeQuorum(uint256 _quorum) external onlyWallet validRequirement(owners.length, _quorum) {
    quorum = _quorum;
  ...
  modifier isTransactionExist(uint256 _transactionId) {
    require(_transactionId < transactions.length, "Treasury: Transaction does not exist.");
    _;
  }

  modifier notExecuted(uint256 _transactionId) {
    require(!transactions[_transactionId].executed, "Treasury: Transaction is already executed.");
    _;
  }

  modifier notConfirmed(uint256 _transactionId) {
    require(!isConfirmed[_transactionId][msg.sender], "Treasury: Transaction is already confirmed");
    _;
  }

  modifier validRequirement(uint256 _ownerCount, uint256 _quorum) {
    require(_ownerCount <= MAX_OWNER_COUNT && _quorum <= _ownerCount && _quorum != 0 && _ownerCount != 0, "Treasury: Invalid Requirement.");
    _;
  }
}
