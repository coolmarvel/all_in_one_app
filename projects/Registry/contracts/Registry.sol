...
contract ContractRegistry is Ownable {
  ...
  modifier isExistAddress(bytes32 _name, address _address) {
    require(contractsAddress[_name] != _address, "Registry: Same address already exists");
    _;
  }
  ...
  function contractDeployed(
    ...
    uint256 _block,
    bytes32 _tx
  ) external onlyOwner isContractAddress(_address) isExistAddress(_name, _address) {
